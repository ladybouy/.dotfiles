...
enum { SchemeNorm, SchemeSel, SchemeTitle, SchemeIcon }; /* color schemes */
...
drawstatusbar(Monitor *m, int bh, char* stext) {
	...
	while (text[++i]) {
		if (text[i] == '^' && !isCode) {
			...
			text[i] = '\0';
			w = TEXTW(text) - lrpad;
			drw_text(drw, x, 2+vertstatuspad / 2, w, bh - vertstatuspad-3, 0, text, 0);
...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		tw = statusw = m->ww - drawstatusbar(m, bh, stext) - 2 * sp;
	}
	...
	x = 0;
    w = TEXTW(distroicon);
    drw_setscheme(drw, scheme[SchemeIcon]);
	x = drw_text(drw, x, 0, w, bh, lrpad / 2,  distroicon, 0);
...
setup(void)
{
	...
	lrpad = drw->fonts->h;
	bh = drw->fonts->h + 2 + vertstatuspad;
	updategeom();
	sp = sidepad;
