...
if [[ $(cat /sys/class/net/w*/operstate) == "down" ]] && \
    ...
    echo -e $POWERLINE_COLOR$LEFT_ROUND$ICON_BG_COLOR$ICON_COLOR$TEXT_COLOR$ETHERNET_UP $BAR_BG_COLOR$POWERLINE_COLOR_2$RIGHT_ROUND 
elif [[ $(cat /sys/class/net/w*/operstate) == "up" ]] && \
    [[ $(cat /sys/class/net/enp35s0/operstate) == "up" ]]; then
    echo -e $POWERLINE_COLOR$LEFT_ROUND$ICON_BG_COLOR$ICON_COLOR$TEXT_COLOR$ETHERNET_UP $BAR_BG_COLOR$POWERLINE_COLOR_2$RIGHT_ROUND 
elif [[ $(cat /sys/class/net/w*/operstate) == "up" ]] && \
    [[ $(cat /sys/class/net/enp35s0/operstate) == "down" ]]; then
    echo -e $POWERLINE_COLOR$LEFT_ROUND$ICON_BG_COLOR$ICON_COLOR$TEXT_COLOR$WIFI_UP $(awk '/^\s*w/ {print int($3 * 100 /70) "%"}' /proc/net/wireless) $BAR_BG_COLOR$POWERLINE_COLOR_2$RIGHT_ROUND 
else 
    echo -e $ARROW_COLOR'\ue0b2'$BG_COLOR  $TEXT_COLOR$ETHERNET_DOWN$WIFI_DOWN $BAR_BG_COLOR$POWERLINE_COLOR_2$RIGHT_ROUND 
fi
